...
export default function PricingPage() {
  ...
  function handleActivate() {
    ...
    // 匹配 PAPER-XXXXXX-XXXXXX 格式
    if (/^PAPER-[A-F0-9]{6}-[A-F0-9]{6}$/.test(trimmed)) {
  ...
  return (
    <div className="flex-1">
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 page-enter">
        ...
        <p className="text-gray-500 mb-10">选择方案后扫码支付，获取激活码即可解锁全部功能。</p>

        <div className="grid md:grid-cols-3 gap-6 mb-12">
          <PriceCard title="免费试用" price="¥0" desc="新用户2次免费" features={["2次完整使用", "全部功能", "无需激活"]} />
          <PriceCard title="单次购买" price="¥9.9" desc="30天内有效" features={["无限次数使用", "全部功能", "优先响应"]} highlight action="/pay?plan=single" />
          <PriceCard title="月度会员" price="¥29.9" desc="30天无限使用" features={["无限次数使用", "全部功能", "优先响应", "专属客服"]} action="/pay?plan=monthly" />
        </div>

        <div className="p-6 bg-white rounded-xl border border-gray-100 shadow-sm max-w-md mx-auto">
          <h3 className="font-bold text-lg mb-2">已有激活码？</h3>
          <p className="text-sm text-gray-500 mb-4">购买后在此输入激活码即可激活会员</p>
          <div className="flex gap-2">
            <input
              className="flex-1 p-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              placeholder="输入激活码，如 PAPER-XXXXXX-XXXXXX"
              value={code}
...
function PriceCard({ title, price, desc, features, highlight, action }: { title: string; price: string; desc: string; features: string[]; highlight?: boolean; action?: string }) {
  return (
    <div className={`rounded-2xl p-6 border text-left ${highlight ? "border-primary shadow-lg bg-blue-50/50" : "border-gray-200"}`}>
      ...
      {action && (
        <Link href={action} className="block text-center py-2 gradient-primary text-white font-medium rounded-lg text-sm transition">
          立即购买
        </Link>
      )}
    </div>
